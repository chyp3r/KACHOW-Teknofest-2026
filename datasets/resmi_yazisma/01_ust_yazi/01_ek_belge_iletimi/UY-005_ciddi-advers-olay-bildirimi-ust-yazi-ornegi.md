---
id: "UY-005"
kategori: "ust_yazi"
alt_kategori: "01_ek_belge_iletimi"
niyet: "01_ek_belge_iletimi"
baslik: "Ciddi Advers Olay (CAO) bildirimi"
kurum: "Van Eğitim ve Araştırma Hastanesi"
kaynak: "datasets/resmi_yazisma/00_gelen_kaynaklar/ust_yazi/UY-005_ciddi-advers-olay-bildirimi-ust-yazi-ornegi.doc"
yerel_orijinal: "00_gelen_kaynaklar/ust_yazi/UY-005_ciddi-advers-olay-bildirimi-ust-yazi-ornegi.doc"
kaynak_turu: "doc"
belge_turu: "resmi_sablon"
erisim_tarihi: "2026-07-31"
dogrulama: "resmi_kaynaktan_indirildi"
extractor: "antiword"
used_ocr: "false"
page_count: "1"
quality_score: "1.0"
rag_status: "candidate"
kaynak_kurum: "Van Eğitim ve Araştırma Hastanesi"
anonimlestirme_durumu: "uygun"
anonimlestirilen_alan_sayisi: "0"
---

# Ciddi Advers Olay (CAO) bildirimi

T.C. Sağlık Bakanlığı
Türkiye İlaç ve Tıbbi Cihaz Kurumu
İlaç, Biyolojik ve Tıbbi Ürünler Başkan Yardımcılığı
Klinik İlaç Araştırmaları Daire Başkanlığı
ANKARA Tarih

Konu: Ciddi Advers Olay (CAO) bildirimi

Aşağıda detayları verilen araştırmadan, tarafımıza ulaşan CAO formu ve bu
araştırma ile ilgili ülkemizde meydana gelen daha önceki CAO
bildirimlerinin bir özeti ekte bilgilerinize sunulmaktadır.

|Araştırmanın kodu:      |
|Araştırmanın açık adı:       |
|Koordinatörün/sorumlu araştırmacının adı soyadı:      |
|Koordinatör merkezin adresi:      |
|Destekleyicinin adı:      |
|Varsa destekleyicinin yasal temsilcisinin adı:       |
|Ciddi advers olayın meydana geldiği merkez adı:      |
|Ciddi advers olayın meydana geldiği gönüllü no:      |

Kurumumuza gönderilen evraklar ile sistemden eklenen dokümanların birebir
aynı olduğunu ve gerekli elektronik dokümanların arama yapılabilir pdf
olarak hazırlandığını taahhüt ederim.

Gereğini saygılarımla arz ederim.

İMZA
AD-SOYAD

EK:

1. CAO formu (sayfa sayısı belirtilmelidir)

2. Araştırma ile ilgili daha önceki CAO bildirimlerine ait özet tablo

3. İlk uygunluk yazısının bir örneği

4. CAO’ın meydana geldiği merkeze ait uygunluk yazısının bir örneği

5. Etik kurul kararının aslı veya aslı gibidir örneği

NOT: Bu vaka araştırmanın ilk/….. ciddi advers olayıdır.

Bu araştırma ile ilgili daha önceki CAO bildirimlerine ait özet tablo:

|Bildirim |Advers Olayın|Advers Olayın|Nedensellik|Nedensellik |
|Tarihi |Tanımı |Sonucu |İlişkisi |İlişkisinin Kim |
| | | | |Tarafından Yapıldığı|
| | | | | |
| | | | | |
| | | | | |

6.
