...
This module is the deliberate exception, per the tenancy plan's own
diagnosis of that gap (§10): a real Postgres, the real migration chain
(including ``0013_rls``), and two real connection roles.

Lives at ``tests/`` top level, not inside ``tests/integration/``, because
``tests/e2e/`` needs these same fixtures (a real Postgres + RLS is exactly
what an HTTP e2e test needs too) and a non-root ``conftest.py`` cannot be
shared across sibling packages via ``pytest_plugins`` -- pytest 8 forbids
that outside the rootdir conftest. Both ``tests/integration/conftest.py``
and ``tests/e2e/conftest.py`` re-export everything here with
...
#: backend/ -- where alembic.ini lives. This file is backend/tests/_db_fixtures.py.
