...
def create_document_analysis_graph(
    ...
    async def analyze_node(
        ...
        prompt = (
            ...
            # back to official_letter, which the line above names as the default
            # for inter-institution correspondence -- a genelge is structurally an
            # official letter, so only the addressee and the rule-setting language
            # separate the two. Measured on qwen3.5:9b over the sample corpus, this
            # took type accuracy from 11/12 to 12/12, stable across three repeats.
