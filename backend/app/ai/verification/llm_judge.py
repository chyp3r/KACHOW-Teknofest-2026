...
#: Repair guidance per style-check rule id (see
#: ``app.ai.verification.style_checks``), since each rule needs a different
#: instruction for the reviser -- unlike the judge's own findings, which
#: already carry their own ``suggested_fix``.
...
def merge_verdicts(
    ...
    *,
    ...
    Args:
        ...
        style_findings: Register/consistency findings from
            ``app.ai.verification.style_checks`` (person-address
            consistency, repeated filler sentences, a bare placeholder
            label left in the signature block), if any. Folded into the
            score the same way PII/mevzuat findings are, and into
            ``repair_items`` the same way judge findings are -- these are
            exactly the kind of targeted, textual defect the existing
            repair loop already knows how to fix, so no new loop is added
            for them.
