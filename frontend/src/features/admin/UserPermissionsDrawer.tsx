      ...
      {!target ? null : (
        <div className="management-stack">
          ...
          <section>
            ...
            {formOpen && <div className="management-inline-form"><Dropdown label="İşlem" value={action} onChange={(event) => setAction(event.target.value)}>{ACTIONS.map((item) => <option key={item} value={item}>{ACTION_LABELS[item]}</option>)}</Dropdown><Dropdown label="Karar" value={effect} onChange={(event) => setEffect(event.target.value as "permit" | "deny")}><option value="permit">İzin ver</option><option value="deny">Engelle</option></Dropdown><Dropdown label="Kapsam" value={scope} onChange={(event) => setScope(event.target.value as "self" | "any")}><option value="self">Kendi kaynakları</option><option value="any">Şirket genelinde</option></Dropdown><Textarea label="Gerekçe" rows={2} maxLength={500} value={reason} onChange={(event) => setReason(event.target.value)} /><Button loading={grant.isPending} onClick={() => grant.mutate()}>Kaydet</Button></div>}
