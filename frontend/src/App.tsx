    ...
    >
      <Suspense fallback={<PageFallback />}>
        <Routes>
          ...
          <Route path="/documents" element={<DocumentsPage documents={documents.documents} selected={documents.selectedDocument} analysis={documents.analysis} loading={documents.loading} uploading={documents.uploading} updatingFields={documents.updatingFields} deletingDocument={documents.deleting} error={documents.error} onUpload={uploadDocument} onUpdateFields={documents.updateFields} onDeleteDocument={documents.deleteDocument} onGenerateDetailedSummary={documents.generateDetailedSummary} generatingDetailedSummary={documents.generatingDetailedSummary} documentText={documents.documentText} onSaveText={documents.saveText} savingText={documents.savingText} onReextract={documents.reextractText} reextracting={documents.reextracting} onSelect={(document) => { selectDocument(document); navigate(`/documents/${encodeURIComponent(document.storage_path)}`); }} onCloseDocument={() => { documents.setSelectedDocument(null); navigate("/documents"); }} />} />
