...
function draftIssues(draft: PersistedDraft) {
  return [
    ...(draft.missing_information ?? []).map((item) => ({
      title: item.label,
      detail: item.why || "Bu bilgi gönderimden önce doğrulanmalı.",
      tone: "warning" as const,
    })),
    ...(draft.requires_human_approval
      ? [{
          title: "İnsan incelemesi gerekiyor",
          detail: "Taslağın içerik ve dayanaklarını yetkili bir kullanıcı doğrulamalı.",
...
export function DraftsPage({
  ...
  const activeIssues = activeDraft ? draftIssues(activeDraft) : [];
                ...
                {!activeDraftId ? <EmptyState icon={FilePenLine} title="Bir taslak seçin" description="Taslağı incelemek ve sürümlerine erişmek için soldaki listeden seçim yapın." /> : drafts.detailLoading ? <div className="centered-state"><Spinner label="Taslak yükleniyor" />Taslak yükleniyor…</div> : activeDraft ? (
                  <>
                    ...
                    <div className="draft-detail-scroll" role="tabpanel">
                      {detailTab === "draft" ? <div className="draft-review-layout"><DraftDocumentPreview content={activeDraft.content} /><aside className="draft-info-panel" aria-label="Taslak bilgileri"><h3>Taslak bilgileri</h3><dl><div><dt>Kontrol durumu</dt><dd>{activeIssues.length ? <><strong>{activeIssues.length} konu incelenmeli</strong><small>Gönderim öncesinde bulguları doğrulayın</small></> : <strong>Gönderime hazır</strong>}</dd></div><div><dt>Hedef birim</dt><dd><strong>{activeDraft.destination || "Belirtilmedi"}</strong></dd></div><div><dt>Kaynak evrak</dt><dd>{activeDraft.document_id ? <Link to={`/documents/${encodeURIComponent(activeDraft.document_id)}`}>{sourceDocument?.file_name ?? documentName(activeDraft.document_id, documents)}</Link> : <span>Bağlı evrak yok</span>}</dd></div><div><dt>Son güncelleme</dt><dd>{formatDate(activeDraft.updated_at, true)}</dd></div><div><dt>Sürüm</dt><dd>v{activeDraft.version} · {previewVersion ? "Eski sürüm" : "Güncel"}</dd></div></dl></aside></div>
                      : detailTab === "control" ? <div className="draft-control-view"><section className={activeIssues.length ? "needs-review" : "is-ready"}><span>{activeIssues.length ? <AlertTriangle /> : <CheckCircle2 />}</span><div><h3>{activeIssues.length ? `${activeIssues.length} konu incelenmeli` : "Taslak gönderime hazır"}</h3><p>{activeIssues.length ? "Gönderim öncesinde aşağıdaki bulguları doğrulayın." : "Otomatik kontrollerde gönderimi engelleyen bir bulgu bulunmadı."}</p></div></section>{activeIssues.length > 0 && <section><h3>İnceleme başlıkları</h3><ol className="document-issue-list">{activeIssues.map((issue, index) => <li key={`${issue.title}-${index}`} className={`is-${issue.tone}`}><span>{index + 1}</span><div><strong>{issue.title}</strong><p>{issue.detail}</p></div></li>)}</ol>{activeDraft.requires_human_approval && !previewVersion && <footer className="draft-control-review-actions"><p>İçerik ve dayanakları kontrol ettiyseniz insan incelemesini tamamlayın.</p><Button leadingIcon={<CheckCircle2 />} loading={drafts.approvingReview} onClick={() => void drafts.approveReview(activeDraft.id)}>İnceledim, onaylıyorum</Button></footer>}</section>}<section className="draft-confidence-note"><h3>Güven göstergesi</h3><strong>{Math.round(activeDraft.confidence_score ?? 0)} / 100</strong><p>Bu gösterge tek başına doğruluk garantisi değildir; karar için kontrol başlıklarını esas alın.</p></section></div>
                      : detailTab === "routing" ? <DraftRoutingPanel draft={drafts.activeDraft ?? activeDraft} sourceDocumentType={sourceDocument?.document_type} saving={drafts.updatingDestination} onSave={(destination) => void drafts.updateDestination((drafts.activeDraft ?? activeDraft).id, destination)} />
