...
import { PlatformHealthInsights } from "../features/platform/PlatformHealthInsights";
import { PlatformUserInsights } from "../features/platform/PlatformUserInsights";
...
export function PlatformPage() {
  ...
  const companyStats = useQuery({ queryKey: queryKeys.rootCompanies, queryFn: rootService.companies, enabled: tab === "overview" || tab === "companies" || tab === "health" });
  ...
  const userInsights = useQuery({ queryKey: queryKeys.rootUserInsights, queryFn: rootService.userInsights, enabled: tab === "users" });
  ...
  const error = overview.error ?? companyStats.error ?? companies.error ?? companyDetail.error ?? userInsights.error ?? health.error ?? create.error ?? update.error ?? remove.error ?? assignAdmin.error;
  const summary = overview.data;
  const companyNames = Object.fromEntries((companyStats.data ?? []).map((company) => [company.company_id, company.name]));
  return <div className="page page-scroll platform-page"><PageHeader title="Platform Yönetimi" description="Kurumlar arası sistem görünümü ve root yönetim alanı." /><ApiErrorNotice error={error || null} /><Tabs label="Platform yönetimi" active={tab} onChange={setTab} items={[{ id: "overview", label: "Genel Bakış", icon: <LayoutDashboard /> }, { id: "companies", label: "Kurumlar", icon: <Building2 /> }, { id: "users", label: "Kullanıcı İstatistikleri", icon: <Users /> }, { id: "health", label: "Sistem Sağlığı", icon: <HeartPulse /> }, { id: "audit", label: "Denetim", icon: <ScrollText /> }]} />
    ...
    {tab === "users" && <PlatformUserInsights data={userInsights.data} loading={userInsights.isLoading} />}
    {tab === "health" && <PlatformHealthInsights data={health.data} loading={health.isLoading} companyNames={companyNames} checkedAt={health.dataUpdatedAt} />}
