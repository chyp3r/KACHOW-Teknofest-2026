...
- **Taslak Kalıcılığı (`drafts` Tablosu) ve "Son Taslağı Düzenle" Niyeti**: Taslak turlar arasında yaşamıyordu -- `planning_node` her turda `draft_result`'ı sıfırlıyor, `_run_draft` önceki taslağı hiç prompt'a koymuyordu, dolayısıyla "son taslaktaki 'ben' ifadelerini 'biz' yap" dendiğinde sistem sıfırdan yeni bir taslak üretiyordu.
  - Yeni `app/domains/drafts/` domain'i (`DraftModel`, `DraftRepository`, `DraftService`, `GET /drafts`, `GET /drafts/{id}`, `GET /drafts/{id}/versions`) ve `alembic/versions/0002_drafts.py`: her taslak üretimi/revizyonu, `session_id` + artan `version` + `parent_draft_id` zinciriyle append-only bir satır olarak kalıcı hâle geliyor. `checkpoint_memory.py`'nin "ikinci hafıza deposu tutma" ilkesine bilinçli bir istisna -- burada saklanan konuşma geçmişi değil, iş çıktısı (taslak metni); konuşma hafızası tek kaynak olarak checkpoint'te kalmaya devam ediyor.
  - `ChatService` artık kendi `session_factory`'sini taşıyor (varsayılan `AsyncSessionLocal`) -- streaming yanıtlar arka plan `asyncio.create_task`'ında çalıştığı için request-scoped bir `Depends(get_db)` bu noktada zaten kapanmış olurdu. Her turdan önce `_fetch_last_draft` konuşmanın son taslağını çekip `PlanningState.last_draft`'a koyuyor; her turdan sonra `_persist_draft_version` yeni bir versiyon yazıyor (içerik önceki versiyonla aynıysa -- salt bir onay gibi -- atlanıyor). İkisi de DB kesintisinde sessizce `{}`'e/no-op'a düşüyor, turu asla başarısız etmiyor.
  - `PlanningState.last_draft`: `planning_node`'un sıfırladığı `<step>_result` alanlarının aksine bilerek sıfırlanmıyor -- `input_text`/`document_id` gibi çağıranın verdiği ham bir alan.
  - Yeni `draft_revision` niyeti (`intent_rules.py`, `EvidenceRule.requires_last_draft`): `PLAN_BY_INTENT["draft_revision"] = ["draft"]` -- classification ve routing çalışmadan doğrudan taslak adımına gidiyor. `draft_graph.py`'nin `writer_node`'u zaten `previous_draft` set edildiğinde reviser'a atlıyordu (verify-tetiklemeli revizyon döngüsünün parçası olarak); yeni `revision_instruction` alanı, bu girişte henüz bir kusur listesi olmadığında revizör'e ne değişeceğini söylüyor. `_run_draft_revision` (planning_graph.py), orijinal belgenin bağlamını `last_draft.document_id`'nin önbelleğinden yeniden yüklüyor (bu turun kendi `document_id`'si yerine -- bir revizyon mesajının genelde eki yok).
  - `requires_last_draft=True` kuralları, taslak yokken hiç ateşlenmiyor -- bugünkü davranış (draft_revision'sız) birebir korunuyor. `assist.short_message` sezgiseli, `draft_revision` kanıtı zaten ateşlendiğinde bastırılıyor ("taslağı kısalt" 3 kelime -- bastırma olmasa hem açık ifade (3.0) hem kısalık ipucu (2.0) birikip 1.0'lık, DECISIVE_MARGIN altı bir marj bırakırdı).
  - `chat/router.py`: blanket `dependencies=[Depends(require_auth_if_enabled)]` yerine her endpoint'te isimlendirilmiş `current_user` parametresi -- `REQUIRE_AUTH` açıkken kimliği doğrulanmış kullanıcı artık taslak versiyonuna yazılıyor.
  - **Doğrulama:** `alembic upgrade head` uygulandı; `docker compose run --rm backend pytest -q` → **846 passed** (PR1'in 815'i + 31 yeni: `test_draft_repository.py`, `test_intent_scorer.py`/`test_planner.py`/`test_draft_loop.py`/`test_chat.py`'ye eklenen `draft_revision`/kalıcılık testleri, yeni `test_draft_revision_flow.py`). `python -c "from app.main import app; app.openapi()"` ile `/api/v1/drafts*` uç noktalarının kayıtlı olduğu doğrulandı.
