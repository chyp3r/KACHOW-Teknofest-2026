...
- **Demo şirket + `root` hesabı seed'i**: `app.domains.companies.seeder.seed_demo_company` (idempotent, slug bazlı) her açılışta önce çalışıyor; `app.domains.users.seeder` artık `root@kachow.local` (şirketsiz) + demo şirkete bağlı admin/manager/employee hesaplarını seed'liyor; `app.domains.units.seeder` demo şirkete bağlı 6 varsayılan birimi seed'liyor. Jüri tek tıkla giriş yapmaya devam edebiliyor.
