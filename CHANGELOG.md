...
### Test
- `writing_brief` çözümleyicisi (üç kademeli öneri davranışı dahil), taslak öncesi kapı (duraklama/resume/reddetme/tam belirlenmiş turun atlaması), briefin revize turuna taşınması, `textDiff` kelime-diff yardımcı fonksiyonu ve `PromptQuestionCard`/`DraftMetaStrip`/`InterruptPanel` için kapsamlı birim ve entegrasyon testleri eklendi.
